...
const RELATED: Array<{img: string; cat: string; t: string; to: string}> = [
  {img: bpLayers, cat: 'Design', t: 'The three layers every room needs', to: '/blogs'},
  {
    img: bpRetrofit,
    cat: 'Energy',
    t: 'LED retrofits that pay for themselves',
    to: `/blogs/${BLOG_HANDLE}/led-retrofits-that-pay`,
  },
  {
    img: bpTitle24,
    cat: 'Code',
    t: 'What Title 24 means for your remodel',
    to: `/blogs/${BLOG_HANDLE}/what-title-24-means`,
  },
];
...
async function loadCriticalData({context, request, params}: Route.LoaderArgs) {
  ...
  if (placeholder && blogHandle === BLOG_HANDLE) {
    return {article: placeholder, blogHandle: BLOG_HANDLE};
  }
...
export default function Article() {
  ...
  // Placeholder entries have no Shopify blog index, so send them to /blogs.
  const backTo = blogHandle === BLOG_HANDLE ? '/blogs' : `/blogs/${blogHandle}`;
