...
The store is on **Basic**, so native B2B (companies/catalogs/price lists) is unavailable. B2B pricing = **a percent on the customer + a Shopify Functions discount**:

- **Admin:** the customer metafield **`custom.b2b_discount`** ("B2B discount %", `number_integer` 0–100, pinned on the customer page, customer-account access READ). Type e.g. `20` on a customer → they get 20 % off everything when signed in; empty/0 = retail. "Segments" are just customers sharing a value (build an admin segment on the field to bulk-edit a group). No codes, no tier tables.
- **App `b2b-app/`** (`la-lighting-b2b`, client_id `3188f87b…`, Dev Dashboard org `203205869`, app `422245761025`) — a separate npm project inside this repo (own `package-lock.json`; CI ignores it). Extensions: `extensions/b2b-pricing` = discount function (JS, target `cart.lines.discounts.generate.run`) that reads `cart.buyerIdentity.customer.metafield(custom.b2b_discount)` and adds a `productDiscountsAdd` of that percent on every line; `extensions/b2b-pricing-settings` = admin UI block on the discount page (static explainer, nothing to configure). Commands (Node 22, from `b2b-app/`): `shopify app build`, `npx vitest run` in the function dir (fixtures in `tests/fixtures/`), `shopify app deploy --allow-updates` (releases to the store). **The store is not a dev store**, so `shopify app dev` / `app execute -s …` refuse it — install/upgrade go through Dev Dashboard → Distribution (custom, single store) → install link; the discount itself is created by the merchant in Admin → Discounts → Create → "B2B pricing" (app) → Save.
- **Storefront:** root loader exposes `isLoggedIn` + `b2b` (the percent; Customer Account API read of the metafield, cached in the session as `b2b`, cleared in `account_.authorize`; `app/lib/b2b.server.ts`). `useB2B()` / `<B2BMoney>` / `b2bAmount(amount, pct)` in `app/lib/b2b.tsx` render the discounted price with the list price struck through on PDP/catalog/cards/search/home slider. The `/cart` action ends with `attachCustomerToCart` (ties a guest-started cart to the login so the function sees the customer); cart lines strike the pre-discount `subtotalAmount`. `CART_MUTATE_FRAGMENT` adds `buyerIdentity.customer` for that check.
