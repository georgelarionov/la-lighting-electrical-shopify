...
**Homepage** (`app/routes/_index.tsx`) composes section components from `app/components/sections/*` — it is a 1:1 port of the Pencil design in `design/design.pen` (the source of truth; `HANDOFF.md` is the port plan). The "Shop the catalog" slider (featured **products** from the built-in `frontpage` / "Home page" collection — the merchant curates membership + order in admin, no code change) and Blog are data-driven from the `_index` loader; other sections are static. Marketing copy/nav/contact live in `app/lib/site.ts`; form validators in `app/lib/home-forms.ts` (the quote form posts to the `_index` action; newsletter + referral post to the `app/routes/api.subscribe.tsx` resource route). **Imagery is self-hosted** in `app/assets/*.jpg`, imported via `?url` and rendered with plain `<img>` (same-origin — no CSP change needed). Real Shopify product/collection/article images load from `cdn.shopify.com` via Hydrogen `<Image>`; arbitrary external image hosts are blocked by the CSP in `app/entry.server.tsx`.
...
The store is on **Basic**, so native B2B (companies/catalogs/price lists) is unavailable. B2B pricing = **plain `custom` metafields the merchant edits in admin + a Shopify Functions discount** that applies them in the cart. Three knobs, no codes, no code changes:

- **Groups:** shop metafield **`custom.b2b_groups`** ("B2B groups", `json`, storefront PUBLIC_READ; Settings → Custom data → Shop) — group key → percent off everything, e.g. `{"contractor": 25, "wholesale": 35}`.
- **Customer:** **`custom.b2b_group`** ("B2B group", text = a key from that JSON) and/or **`custom.b2b_discount`** ("B2B discount %", integer 0–100 — this customer's own percent, **beats the group's**). Both pinned on the customer page, customer-account access READ. Empty = retail. To bulk-assign a group, build an admin segment and bulk-edit the field.
- **Per-variant prices:** variant metafield **`custom.b2b_prices`** ("B2B prices", `json`, storefront PUBLIC_READ; product → variant) — per **group key** or per **customer ID** (the number in the customer's admin URL): a **fixed price** (number) or a **percent off** (string ending in `%`), e.g. `{"contractor": 12.5, "wholesale": "30%", "9588593000471": 11}`. Beats both percents; the customer-ID entry beats the group entry; a fixed price at/above list = no discount (a discount function cannot raise prices; the store is single-currency, so fixed prices are in USD).
- **App `b2b-app/`** (`la-lighting-b2b`, client_id `3188f87b…`, Dev Dashboard org `203205869`, app `422245761025`) — a separate npm project inside this repo (own `package-lock.json`; CI ignores it). Extensions: `extensions/b2b-pricing` = discount function (JS, target `cart.lines.discounts.generate.run`) whose input query reads the four metafields above plus `lines.cost.amountPerQuantity` (input-query cost cap is 30, size 3000 bytes) and emits one `productDiscountsAdd` candidate per line (`percentage` or `fixedAmount` = list − fixed price, `appliesToEachItem`); `extensions/b2b-pricing-settings` = admin UI block on the discount page (static explainer of where each setting lives). Commands (Node 22, from `b2b-app/`): `shopify app build`; in the function dir `npm run typegen` after editing the input query and `npx vitest run` (fixtures in `tests/fixtures/`, one JSON per scenario); `shopify app deploy --allow-updates` (releases to the store). **The store is not a dev store**, so `shopify app dev` / `app execute -s …` refuse it — install/upgrade go through Dev Dashboard → Distribution (custom, single store) → install link; the discount itself is created by the merchant in Admin → Discounts → Create → "B2B pricing" (app) → Save.
- **Storefront:** root loader exposes `isLoggedIn` + `b2b: {id, group, pct} | null` (`app/lib/b2b.server.ts` `getB2B`: Customer Account API read of id/group/discount once per login, cached in the session as `b2b`, cleared in `account_.authorize`; the group registry is a `CacheShort` Storefront read so edits show without re-login). `app/lib/b2b.tsx`: `b2bRule` (**mirrored in the function — keep in sync**), `b2bPrice(list, variant.b2bPrices?.value, b2b)`, `<B2BMoney data prices?>`, `useB2B()`, `b2bLabel()`. PDP passes the selected variant's `b2bPrices` metafield (exact price); catalog/search/home cards have no variants and apply only the percent ("From …"). The `/cart` action ends with `attachCustomerToCart` (ties a guest-started cart to the login so the function sees the customer); cart lines strike the pre-discount `subtotalAmount`. `CART_MUTATE_FRAGMENT` adds `buyerIdentity.customer` for that check.
...
- The earlier tag `b2b` + code `B2B` model is retired: the code discount is deactivated (EXPIRED), the "B2B customers" segment still exists but drives nothing.
